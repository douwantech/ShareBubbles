...
extension ShareAttirbute {
    ...
    private static func createButton(bubbleId: Int, imageName: String, color: UIColor) -> ShareAttirbute? {
        ...
        guard let bundle = Bundle(path: "\(classBundle.bundlePath)/ShareBubbles.bundle") else { return nil }
        if let icon = UIImage(named: "ShareBubbles.bundle/\(imageName)", in: bundle, compatibleWith: nil) {
